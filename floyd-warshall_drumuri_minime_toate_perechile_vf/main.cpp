...
void floyd_warshall(vector<vector<int>>& d, vector<vector<int>>& p)
{
    ...
    {
        for(int i = 1; i <= n; i++) // nod sursa
        {
            for(int j = 1; j <= n; j++) // nod destinatie
            {
...
void afiseaza_drumul(int u, int v, const vector<vector<int>>& p)
{
    if(p[u][v] == -1)
    {
...
int main()
{
    ...
    vector<vector<int>> p(n + 1, vector<int>(n + 1, -1)); // matrice predecesori
